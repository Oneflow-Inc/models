...
def plot_loss(root_dir, epoch):
    gen_image_loss = np.load(os.path.join(
        root_dir, 'G_image_loss_{}.npy'.format(epoch)))
    gen_gan_loss = np.load(os.path.join(
        root_dir, 'G_GAN_loss_{}.npy'.format(epoch)))
    gen_total_loss = np.load(os.path.join(
        root_dir, 'G_total_loss_{}.npy'.format(epoch)))
    ...
    for lo, na in zip(loss, name):
        ...
        if isinstance(lo, list):
            ...
            plt.savefig(os.path.join(
                root_dir, 'image_loss_{}.png'.format(epoch)))
        else:
            plt.plot(lo, label=na)
            plt.legend()
            plt.savefig(os.path.join(
                root_dir, '{}_loss_{}.png'.format(na, epoch)))
