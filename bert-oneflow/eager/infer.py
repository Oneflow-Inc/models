...
def _parse_args():
    ...
    parser.add_argument("--model-path", type=str, metavar="DIR", help="model path")
    ...
    parser.add_argument(
        "--layers", type=int, default=8, help="number of layers in transfomer model"
    )
    parser.add_argument(
        "--attn-heads", type=int, default=8, help="number of attention heads"
    )
    parser.add_argument(
...
def inference(args):
    ...
    start_t = time.time()
    inputs = [[0, 1, 2]]
    inputs = flow.Tensor(inputs, dtype=flow.int32, device=flow.device(args.device))
    mask = inputs > 0

    segment_info = flow.zeros_like(inputs)
    prediction = bert_module.forward(inputs, mask, segment_info).numpy()
    print(prediction)
    end_t = time.time()
