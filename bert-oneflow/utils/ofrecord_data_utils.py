...
class OfRecordDataLoader(nn.Module):
    ...
    def forward(self):
        ...
        if self.use_consistent:
            input_ids = input_ids.to_consistent(placement=self.placement, sbp=self.sbp)
            next_sent_labels = next_sent_labels.to_consistent(placement=self.placement, sbp=self.sbp)
            input_mask = input_mask.to_consistent(placement=self.placement, sbp=self.sbp)
            segment_ids = segment_ids.to_consistent(placement=self.placement, sbp=self.sbp)
            masked_lm_ids = masked_lm_ids.to_consistent(placement=self.placement, sbp=self.sbp)
            masked_lm_positions = masked_lm_positions.to_consistent(placement=self.placement, sbp=self.sbp)
            masked_lm_weights = masked_lm_weights.to_consistent(placement=self.placement, sbp=self.sbp)
