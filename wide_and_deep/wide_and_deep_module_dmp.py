...
class WideAndDeep(nn.Module):
    ...
    def forward(self, dense_fields, wide_sparse_fields, deep_sparse_fields) -> flow.Tensor:
        ...
        wide_scores=wide_scores.to_consistent(placement=placement,sbp=flow.sbp.split(0)).to("cuda")
        ...
        deep_features=deep_features.to_consistent(placement=placement,sbp=flow.sbp.split(0)).to("cuda")
        ...
        predicts=predicts.to_consistent(placement=placement,sbp=flow.sbp.broadcast).to("cuda")
        return predicts
