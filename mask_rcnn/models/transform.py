...
def _resize_image_and_masks_onnx(image, self_min_size, self_max_size, target):
    ...
    scale_factor = flow.min(self_min_size / min_size, self_max_size / max_size)
...
class GeneralizedRCNNTransform(nn.Module):
    ...
    def normalize(self, image):
        ...
        mean = flow.Tensor(self.image_mean, dtype=dtype, device=device)
        std = flow.Tensor(self.image_std, dtype=dtype, device=device)
        return (image - mean[:, None, None]) / std[:, None, None]

    def flow_choice(self, k):
        # type: (List[int]) -> int
        """
        Implements `random.choice` via torch detection_ops so it can be compiled with
    ...
    def resize(self, image, target):
        ...
        h, w = image.shape[-2:]
        if self.training:
            size = float(self.flow_choice(self.min_size))
        else:
    ...
    def _onnx_batch_images(self, images, size_divisible=32):
        ...
        for i in range(images[0].dim()):
            max_size_i = flow.max(flow.stack([img.shape[i] for img in images]).to(flow.float32)).to(flow.int64)
            max_size.append(max_size_i)
        stride = size_divisible
        max_size[1] = (flow.ceil((max_size[1].to(flow.float32)) / stride) * stride).to(flow.int64)
        max_size[2] = (flow.ceil((max_size[2].to(flow.float32)) / stride) * stride).to(flow.int64)
        ...
        for img in images:
            ...
            padded_img = nn.functional.pad(img, (0, padding[2], 0, padding[1], 0, padding[0]))
            padded_imgs.append(padded_img)

        return flow.stack(padded_imgs)
...
def resize_boxes(boxes, original_size, new_size):
    ...
    return flow.stack((xmin, ymin, xmax, ymax), dim=1)
