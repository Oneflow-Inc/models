...
#     num_keypoints = torch.scalar_tensor(maps.size(1), dtype=flow.int64)
#
...
def heatmaps_to_keypoints(maps, rois):
    ...
    xy_preds = flow.zeros((len(rois), 3, num_keypoints), dtype=flow.float32, device=maps.device)
    end_scores = flow.zeros((len(rois), num_keypoints), dtype=flow.float32, device=maps.device)
    for i in range(len(rois)):
        ...
        xy_preds[i, 2, :] = 1
        end_scores[i, :] = roi_map[flow.arange(num_keypoints), y_int, x_int]
...
def keypointrcnn_loss(keypoint_logits, proposals, gt_keypoints, keypoint_matched_idxs):
    ...
    keypoint_targets = flow.cat(heatmaps, dim=0)
    valid = flow.cat(valid, dim=0).to(dtype=flow.uint8)
    valid = flow.where(valid)[0]
    ...
    keypoint_loss = nn.cross_entropy(keypoint_logits[valid], keypoint_targets[valid])
...
def _onnx_expand_boxes(boxes, scale):
    ...
    w_half = w_half.to(dtype=flow.float32) * scale
    h_half = h_half.to(dtype=flow.float32) * scale

    boxes_exp0 = x_c - w_half
    boxes_exp1 = y_c - h_half
    boxes_exp2 = x_c + w_half
    boxes_exp3 = y_c + h_half
    boxes_exp = flow.stack((boxes_exp0, boxes_exp1, boxes_exp2, boxes_exp3), 1)
    return boxes_exp
...
def expand_masks_tracing_scale(M, padding):
    # type: (int, int) -> float
    return flow.tensor(M + 2 * padding).to(flow.float32) / flow.tensor(M).to(flow.float32)
...
def _onnx_paste_mask_in_image(mask, box, im_h, im_w):
    one = flow.ones(1, dtype=flow.int64)
    zero = flow.zeros(1, dtype=flow.int64)

    w = (box[2] - box[0] + one)
    h = (box[3] - box[1] + one)
    w = flow.max(flow.cat((w, one)))
    h = flow.max(flow.cat((h, one)))
    ...
    x_0 = flow.max(flow.cat((box[0].unsqueeze(0), zero)))
    x_1 = flow.min(flow.cat((box[2].unsqueeze(0) + one, im_w.unsqueeze(0))))
    y_0 = flow.max(flow.cat((box[1].unsqueeze(0), zero)))
    y_1 = flow.min(flow.cat((box[3].unsqueeze(0) + one, im_h.unsqueeze(0))))
    ...
    # pad y
    zeros_y0 = flow.zeros(y_0, unpaded_im_mask.size(1))
    zeros_y1 = flow.zeros(im_h - y_1, unpaded_im_mask.size(1))
    concat_0 = flow.cat((zeros_y0,
                          unpaded_im_mask.to(dtype=flow.float32),
                          zeros_y1), 0)[0:im_h, :]
    # pad x
    zeros_x0 = flow.zeros(concat_0.size(0), x_0)
    zeros_x1 = flow.zeros(concat_0.size(0), im_w - x_1)
    im_mask = flow.cat((zeros_x0,
                         concat_0,
                         zeros_x1), 1)[:, :im_w]
    return im_mask
...
def _onnx_paste_masks_in_image_loop(masks, boxes, im_h, im_w):
    res_append = flow.zeros(0, im_h, im_w)
    for i in range(masks.size(0)):
        mask_res = _onnx_paste_mask_in_image(masks[i][0], boxes[i], im_h, im_w)
        mask_res = mask_res.unsqueeze(0)
        res_append = flow.cat((res_append, mask_res))
    return res_append
