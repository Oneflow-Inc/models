...
from oneflow.experimental import F
...
class AdaptiveInstanceNorm2d(nn.Module):
    ...
    def forward(self, x):
        ...
        import warnings
        warnings.warn("don't know whether 'axis' should be 0 or 1")
        out = F.normalization(
            x_reshaped, running_mean, running_var, self.weight, self.bias,
            axis=0, training=True, momentum=self.momentum, eps=self.eps)
        return out.reshape((b, c, x.shape[2], x.shape[3]))

    def __repr__(self):
        return f"{type(self)}({self.num_features})"
