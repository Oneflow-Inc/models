...
def _parse_args():
    ...
    parser.add_argument(
        "--model_path",
        type=str,
        default="./0.15dcgan/checkpoint/g_99",
        help="path to trained generator",
    )
    parser.add_argument(
        "--save_path", type=str, default="test_images.png", help="path to save"
    )
    parser.add_argument("--batch_size", type=int, default=16, help="val batch size")
    parser.add_argument(
...
def main(args):
    ...
    start_t = time.time()
    z = to_tensor(np.random.normal(0, 1, size=(args.batch_size, 100)), False).to(device)
