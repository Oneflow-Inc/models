...
def train(args):
    ...
    with make_criteo_dataloader(
        ...
        for step in range(1, args.train_batches + 1):
            ...
            logits = dlrm_module(dense_fields.to("cuda"), sparse_fields.to("cuda"))
            ...
            if args.eval_interval > 0 and step % args.eval_interval == 0:
                auc = eval(cached_eval_batches, dlrm_module, step)
...
def eval(cached_eval_batches, module, cur_step=0):
    ...
    for i in range(num_eval_batches):
        ...
        with flow.no_grad():
            logits = module(dense_fields.to("cuda"), sparse_fields.to("cuda"))
            pred = logits.sigmoid()
        labels.append(label)
