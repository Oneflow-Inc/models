...
class MLP(nn.Module):
    def __init__(self, in_features: int, hidden_units) -> None:
        ...
        self.linear_layers = nn.Sequential(
            OrderedDict(
                [
                    (f"fc{i}", Dense(in_features if i == 0 else hidden_units[i - 1], h))
...
class Interaction(nn.Module):
    def __init__(
        self, interaction_type="dot", interaction_itself=False, num_sparse_fields=26
    ):
    ...
    def output_feature_size(self, embedding_vec_size, dense_feature_size):
        if self.interaction_type == "dot":
            ...
            assert (
                embedding_vec_size == dense_feature_size
            ), "Embedding vector size must equle to dense feature size"
...
class Embedding(nn.Embedding):
    def __init__(self, vocab_size, embed_size, args):
        if args.is_global:
            assert (
                args.embedding_split_axis < 2
            ), "Embedding model parallel split axis can only be 0 or 1."
...
class OneEmbedding(nn.Module):
    ...
    def forward(self, ids):
        bsz = ids.shape[0]
        column_id = (
            flow.ones((bsz, 1), dtype=flow.int32, sbp=ids.sbp, placement=ids.placement)
            * self.column_id
        )
        if ids.is_global:
...
class DLRMModule(nn.Module):
    def __init__(self, args):
        ...
        self.embedding = embd_dict[args.embedding_type](
            args.vocab_size, args.embedding_vec_size, args
        )
        self.interaction = Interaction(
            args.interaction_type, args.interaction_itself, args.num_sparse_fields
        )
        feature_size = self.interaction.output_feature_size(
