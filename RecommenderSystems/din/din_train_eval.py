...
class xDeepFMModule(nn.Module):
    def __init__(
        ...
        self.hist_embedding_layer = OneEmbedding(
            table_name="hist_sparse_embedding",
        ...
        self.tar_seq_embedding_layer = OneEmbedding(
            table_name="tar_seq_sparse_embedding",
            embedding_vec_size=embedding_vec_size,
        ...
        self.tar_embedding_layer = OneEmbedding(
            table_name="tar_sparse_embedding",
            embedding_vec_size=embedding_vec_size,
        ...
        self.tar_b_embedding_layer = OneEmbedding(
    ...
    def forward(self, inputs) -> flow.Tensor:
        ...
        hist_seq_concat = flow.concat([inputs[0], inputs[1]], dim = -1)
        hist_seq_concat = self.hist_embedding_layer(hist_seq_concat).reshape([bs, max_len, -1])

        target_seq_concat = flow.concat([inputs[5], inputs[6]], dim=-1)
        target_seq_concat = self.tar_seq_embedding_layer(target_seq_concat).reshape([bs, max_len, -1])

        target_concat = flow.concat([inputs[2].reshape([bs, 1]), inputs[3].reshape([bs, 1])], dim=-1)
        target_concat = self.tar_embedding_layer(target_concat).reshape([bs, -1])

        item_b = self.tar_b_embedding_layer(inputs[2])
