...
class OneEmbedding(nn.Module):
    def __init__(
        ...
        cache_memory_budget_mb,
        size_factor,
        snapshot_name = "2022-07-14-21-53-04-270525",
    ):
...
class DINModule(nn.Module):
    def __init__(
        ...
        self.hist_item_emb_attr = OneEmbedding(
            ...
            size_factor=size_factor,
            snapshot_name="2022-07-14-21-53-04-270525-0",
        ) 
    ...
    def forward(self, inputs) -> flow.Tensor:
        ...
        # hist_item_emb = flow.gather(self.id_embedding_dict[0], -1, hist_item_seq)
        # hist_cat_emb = flow.gather(self.cat_embedding_dict[0], -1, hist_cat_seq)
        # target_item_emb = flow.gather(self.id_embedding_dict[1], -1, target_item)
        # target_cat_emb = flow.gather(self.cat_embedding_dict[1], -1, target_cat)
        # target_item_seq_emb = flow.gather(self.id_embedding_dict[2], -1, target_item_seq)
        # target_cat_seq_emb = flow.gather(self.cat_embedding_dict[2], -1, target_cat_seq)
        # item_b = flow.gather(self.item_b_attr, -1, target_item)
