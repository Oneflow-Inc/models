...
def split_criteo(args):
    ...
    test_df.to_csv(
        os.path.join(args.output_data_dir, "test.csv"), index=False, encoding="utf-8"
    )
    valid_df = ddf.loc[valid_index, :]
    valid_df.to_csv(
        os.path.join(args.output_data_dir, "valid.csv"), index=False, encoding="utf-8"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--criteo_txt_path",
        type=str,
        default="dac/train.txt",
        help="the raw Criteo data",
    )
    parser.add_argument(
        "--output_data_dir",
        type=str,
