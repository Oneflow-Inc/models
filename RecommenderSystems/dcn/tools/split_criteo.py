...
def split_criteo(args):
    ...
    ddf = pd.read_csv(criteo_txt_path, sep='\t', header=None, names=cols, encoding='utf-8', dtype=object)
    X = ddf.values
    ...
    ddf.loc[train_index, :].to_csv(os.path.join(args.output_data_dir,'train.csv'), index=False, encoding='utf-8')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--criteo_txt_path", type=str, default='dac/train.txt', help="the raw Criteo data")
    parser.add_argument("--output_data_dir", type=str, default='../Criteo/Criteo_csv', help="the splited data output directory")
