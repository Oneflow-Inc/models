...
class Tokenizer(object):
    ...
    def encode_sequence(self, texts):
        ...
        for text in texts:
            if pd.isnull(text) or text == "":
                ...
                sequence_list.append(
                    [self.vocab.get(x, self.oov_token) for x in text.split(self._splitter)]
                )
        sequence_list = padding(
            sequence_list,
            maxlen=self.max_len,
    ...
    def load_pretrained_embedding(self, feature_name, pretrain_path, embedding_dim, output_path):
        ...
        embedding_matrix = np.random.normal(
            loc=0, scale=1.0e-4, size=(self.vocab_size, embedding_dim)
        )
...
def padding(sequences, maxlen=None, dtype="int32", padding="pre", truncating="pre", value=0.0):
