...
def train(args):
    ...
    one_embedding_opt = flow.one_embedding.Optimizer(
        opt, embeddings=[dcn_module.embedding_layer.one_embedding]
    )

    loss_fn = flow.nn.BCELoss(reduction="mean").to("cuda")
    ...
    with make_criteo_dataloader(
        ...
        for step in range(1, args.train_batches + 1):
            ...
            logits = dcn_module(features.to("cuda"))
            loss = loss_fn(logits, labels.to("cuda"))
            loss.backward()
            one_embedding_opt.step()
            one_embedding_opt.zero_grad()
    ...
    eval(args, dcn_module, tag="test", cur_step=step, epoch=epoch)
...
def eval(args, eval_module, tag="val", cur_step=0, epoch=0, cached_eval_batches=None):
    if tag == "val":
    ...
    if cached_eval_batches == None:
        with make_criteo_dataloader(f"{args.data_dir}/{tag}", batch_size, shuffle=False) as loader:
            ...
            for i in range(batches_per_epoch):
                with flow.no_grad():
                    label, features = batch_to_global(*next(loader), is_train=False)
                    pred = eval_module(features.to("cuda"))
                    labels.append(label)
