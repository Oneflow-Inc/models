...
class InnerProductLayer(nn.Module):
    ...
    def __init__(self, num_fields=None, output="product_sum_pooling"):
        ...
        # self.field_p = nn.Parameter(flow.LongTensor(p), requires_grad=False)
        # self.field_q = nn.Parameter(flow.LongTensor(q), requires_grad=False)
        self.interaction_units = int(num_fields * (num_fields - 1) / 2)
        # self.upper_triange_mask = nn.Parameter(flow.triu(flow.ones(num_fields, num_fields), 1).type(flow.ByteTensor),
        # self.upper_triange_mask = nn.Parameter(flow.triu(flow.ones(num_fields, num_fields), 1), 
        #                                            requires_grad=False)
        self.upper_triange_mask = flow.triu(flow.ones(num_fields, num_fields), 1).to_global(placement=flow.env.all_device_placement("cuda"), sbp=flow.sbp.split(0))

    def forward(self, feature_emb):

        inner_product_matrix = flow.bmm(feature_emb, feature_emb.transpose(1, 2))
        print("inner_product_matrix.shape: ", inner_product_matrix.shape)

        print("upper_triange_mask.shape: ", self.upper_triange_mask.shape)
        # flat_upper_triange = [flow.masked_select(_, self.upper_triange_mask) for _ in inner_product_matrix]
        flat_upper_triange = flow.masked_select(inner_product_matrix, self.upper_triange_mask.unsqueeze(0))
        print(len(flat_upper_triange))
...
class PNNModule(nn.Module):
    def __init__(
        ...
        self.inner_product_layer = InnerProductLayer(num_fields, output="inner_product")
        self.input_dim = int(num_fields * (num_fields - 1) / 2) \
                  + num_fields * embedding_vec_size
        self.dnn_layer = DNN(
    ...
    def forward(self, inputs) -> flow.Tensor:
        ...
        inner_product_vec = self.inner_product_layer(embedded_x)
        dense_input = flow.cat([embedded_x.flatten(start_dim=1), inner_product_vec], dim=1)
        y_pred = self.dnn_layer(dense_input)
