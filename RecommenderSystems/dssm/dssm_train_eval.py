...
class DssmValGraph(flow.nn.Graph):
    def __init__(self, dssm_module, amp=False):
        super(DssmValGraph, self).__init__()
...
def train(args):
    ...
    dssm_module = make_dssm_module(args)
    dssm_module.to_global(flow.env.all_device_placement("cuda"), flow.sbp.broadcast)
    ...
    def load_model(dir):
        ...
        if os.path.exists(dir):
            ...
            dssm_module.load_state_dict(state_dict, strict=False)
        else:
            if rank == 0:
    ...
    def save_model(subdir):
        ...
        state_dict = dssm_module.state_dict()
    ...
    opt = flow.optim.Adam(dssm_module.parameters(), lr=args.learning_rate)
