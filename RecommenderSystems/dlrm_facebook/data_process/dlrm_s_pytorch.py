...
class DLRM_Net(nn.Module):
    ...
    def interact_features(self, x, ly):
        ...
        if self.arch_interaction_op == "dot":
            ...
            #x: (b,64) ly:(b,26,64) dim为1，则是在dim=1方向上扩展，那么ly也是batch_size,? ,这里？应该是
            #26*sparse_dim，经过view之后，应该是batch_size，26+1，dense_dim
            T = torch.cat([x] + ly, dim=1).view((batch_size, -1, d))
            # perform a dot product，计算两个tensor的矩阵乘法，torch.bmm(a,b),tensor a 的size为(b,h,w),tensor b的size为(b,w,h),注意两个tensor的维度必须为3.，batch_size，dense_dim，27
            #Z: batch_size,27,27
            ...
            # approach 2: custom ,arch_interaction_itself：和不和自己做点积，默认是不
...
def run():
    ...
    if args.debug_mode:
        ...
        for j, inputBatch in enumerate(train_ld):
            '''b[0], b[1], b[2], b[3], torch.ones(b[3].size()), None'''
            '''
            X.shape torch.Size([50, 13])
            lS_o.shape torch.Size([26, 50]) 不知道这个o是干嘛的，每一行都是1,2...50
    ...
    global dlrm
    '''
    m_spa  sparse feature size(= dimensions of embeddings )
    ln_emb # of embeddings (= # of sparse features) 
    ln_bot bot mlp结构
    ln_top top mlp结构
    arch_interaction_op   parser.add_argument("--arch-interaction-op", type=str, choices=["dot", "cat"], default="dot")
    arch_interaction_itself if args.arch_interaction_itself:
            num_int = (num_fea * (num_fea + 1)) // 2 + m_den_out
        else:
            num_int = (num_fea * (num_fea - 1)) // 2 + m_den_out
    sigmoid_bot=-1, 在哪个地方sigmoid
    sigmoid_top=ln_top.size - 2, 在哪个地方sigmoid
    sync_dense_params=args.sync_dense_params,  应该是在数据并行部分，更新mlp参数
    loss_threshold  loss clamp
    ...
    if args.inference_only:
        ...
        if args.quantize_emb_with_bit != 32:
            dlrm.ƒ(args.quantize_emb_with_bit)
            # print(dlrm)
