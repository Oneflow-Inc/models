    ...
    if epoch % N_eval_epoch == 0:
        ...
        with flow.no_grad():
            for i in range(snt_te):
                ...
                pout = flow.zeros(
                    N_fr+1, class_lay[-1], dtype=flow.float32).to('cuda')
                count_fr = 0
                count_fr_tot = 0
                while end_samp < signal.shape[1]:
                    ...
                    if count_fr == Batch_dev:
                        inp = flow.Tensor(
                            sig_arr, dtype=flow.float32).to('cuda')
                ...
                if count_fr > 0:
                    inp = flow.Tensor(
                        sig_arr[0:count_fr], dtype=flow.float32).to('cuda')
                ...
                err_sum_snt = err_sum_snt + \
                    (best_class.numpy() != lab[0].numpy())
        ...
        print("epoch %i, loss_tr=%f err_tr=%f loss_te=%f err_te=%f err_te_snt=%f time=%f" % (
            epoch, loss_tot.numpy(), err_tot, loss_tot_dev.numpy(), err_tot_dev, err_tot_dev_snt, final-begin))

        with open(output_folder+"/res.res", "a") as res_file:
            res_file.write("epoch %i, loss_tr=%f err_tr=%f loss_te=%f err_te=%f err_te_snt=%f time=%f\n" % (
                epoch, loss_tot.numpy(), err_tot, loss_tot_dev.numpy(), err_tot_dev, err_tot_dev_snt, final-begin))

        flow.save(MOBILENET_net.state_dict(), os.path.join(
            output_folder, "MOBILENET_net_%d" % epoch))
    else:
        final = time.time()
        print("epoch %i / %i, loss_tr=%f err_tr=%f time=%f" %
