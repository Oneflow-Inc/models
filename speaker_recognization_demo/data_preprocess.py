...
def data_preprocess(data_dir, data_preprocessed_dir):
    ...
    for file in os.listdir(data_dir):
        ...
        if os.path.splitext(file_path)[1] == '.wav':
            ...
            assert txt_path.split('/')[-1] in os.listdir(data_dir), "txt file is not exist."
            ...
            with open(txt_path, 'r') as f:
                ...
                for line in f.readlines():
                    if len(line.strip().split('_')) == 4:
                        ...
                        wave_snip = waveform[int(eval(start) * fs): int(eval(end) * fs)]
                        filename = os.path.splitext(file)[0] + '_' + str(count) +'.wav'

                        sf.write(os.path.join(data_preprocessed_dir, filename), wave_snip, fs)
                        data_list.append([os.path.join(data_preprocessed_dir, filename), id])
                        count += 1
                    else:
                        continue
    return data_list
