...
def get_args():
    parser = argparse.ArgumentParser(
        """Speaker Recognization Demo Infer"""
    )
    parser.add_argument(
        "--label_dict",
        type=str,
    ...
    parser.add_argument(
        "--load_path", type=str, default='save_models/CNN_model'
    )
...
def infer(opt):
    ...
    for wav, label in label_list:
        ...
        err_sum += err
        print("wav_filename: ", wav, "    predicted speaker id: ", (pred + 1).numpy()[0], "    real speaker id: ", lab.long().numpy()[0])
