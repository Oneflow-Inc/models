...
def _parse_args():
    parser = argparse.ArgumentParser("flags for test mobilenet")
    parser.add_argument(
        "--model_path", type=str, default="./mobilenet_oneflow_model", help="model path"
    )
    parser.add_argument(
...
def main(args):
    ...
    mobilenet_module.load_state_dict(pretrain_models)
    end_t = time.time()
    ...
    predictions = mobilenet_module(image).softmax()
