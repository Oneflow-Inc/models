...
class GRU_cell_oneflow(nn.Module):
    ...
    def init_weight(self, low, upper):
        self.inp_W.data.copy_(flow.Tensor(rand(self.input_size, self.hidden_size * 3) * (upper - low) + low))
        self.hid_W.data.copy_(flow.Tensor(rand(self.hidden_size, self.hidden_size * 3) * (upper - low) + low))
        self.inp_b.data.copy_(flow.Tensor(rand(self.hidden_size * 3) * (upper - low) + low))
        self.hid_b.data.copy_(flow.Tensor(rand(self.hidden_size * 3) * (upper - low) + low))
    ...
    def forward(self, x, hidden=None):
        ...
        for t in range(seq_len):
            ...
            h_t_ = flow.tanh(gates_1[:, H_S*2:H_S*3] + r_gate * gates_2[:, H_S*2:H_S*3])
