...
parser.add_argument('--train_mode', default='GPU', type=str,
                    choices=['GPU', 'CPU'], help='using GPU or CPU')
parser.add_argument("--batch_size", type=int, default=256, required=False)
parser.add_argument("--load_checkpoint_G", type=str,
                    default="", help="load checkpoint")
parser.add_argument("--load_checkpoint_D", type=str,
                    default="", help="load checkpoint")
...
if __name__ == '__main__':
    ...
    train_data = NumpyDataLoader(
        dataset_root=opt.data_dir, mode=modes[1], hr_size=HR_SIZE, lr_size=lr_size, batch_size=opt.batch_size)
    val_data = ValDatasetFromFolder(
        opt.data_dir, mode=modes[0], upscale_factor=4)
    start_t = time.time()
    ...
    optimizerG = optim.Adam(
        netG.parameters(), lr=opt.lr, betas=(opt.b1, opt.b2))
    optimizerD = optim.Adam(
        netD.parameters(), lr=opt.lr, betas=(opt.b1, opt.b2))
    ...
    for epoch in range(0, NUM_EPOCHS):
        ...
        with flow.no_grad():
            ...
            for idx in range(len(val_data)):
                ...
                batch_ssim = structural_similarity(
                    _img, fake, multichannel=True)
