...
class SincConv_fast(nn.Module):
    ...
    def __init__(self, out_channels, kernel_size, sample_rate=16000, in_channels=1,
        ...
        # Hamming window
        n_lin=flow.Tensor(np.linspace(0, (self.kernel_size/2)-1, int((self.kernel_size/2))))
        self.window_=0.54-0.46*flow.cos(2*math.pi*n_lin/self.kernel_size)

        # (1, kernel_size/2)
        n = (self.kernel_size - 1) / 2.0
        self.n_ = 2*math.pi*flow.Tensor(np.arange(-n, 0).reshape(1, -1) / self.sample_rate, dtype=flow.float32)
    ...
    def forward(self, waveforms):
        ...
        high = flow.clamp(low + self.min_band_hz + flow.abs(self.band_hz_),self.min_low_hz,self.sample_rate/2)
        band=(high-low)[:,0]

        f_times_t_low = flow.matmul(low, self.n_)
        f_times_t_high = flow.matmul(high, self.n_)

        band_pass_left=((flow.sin(f_times_t_high)-flow.sin(f_times_t_low))/(self.n_/2))*self.window_
        ...
        output = flow.F.conv1d(waveforms, self.filters, stride=[self.stride], padding=[self.padding], dilation=[self.dilation], bias=None, groups=1)
        return output
...
class MLP(nn.Module):
    def __init__(self, options):
        ...
        for i in range(self.N_fc_lay):
            ...
            self.wx[i].weight = nn.Parameter(flow.Tensor(self.fc_lay[i],current_input).uniform_(-np.sqrt(0.01/(current_input+self.fc_lay[i])),np.sqrt(0.01/(current_input+self.fc_lay[i]))))
