    ...
    def forward(self, x):
        ...
        for i in range(self.N_cnn_lay):
            ...
            if self.cnn_use_laynorm[i]:
                if i==0:
                    max_pool1 = nn.MaxPool1d(self.cnn_max_pool_len[i], stride=None, padding=0, dilation=1)
                    x = max_pool1(flow.abs(self.conv[i](x)))
                    x = self.drop[i](self.act[i](self.ln[i](x)))
                else:
                    max_pool2 = nn.MaxPool1d(self.cnn_max_pool_len[i], stride=None, padding=0, dilation=1)
                    x = max_pool2(self.conv[i](x))
                    x = self.drop[i](self.act[i](self.ln[i](x)))

            if self.cnn_use_batchnorm[i]:
                max_pool3 = nn.MaxPool1d(self.cnn_max_pool_len[i], stride=None, padding=0, dilation=1)
                x = max_pool3(self.conv[i](x))
                x = self.drop[i](self.act[i](self.bn[i](x)))

            if self.cnn_use_batchnorm[i]==False and self.cnn_use_laynorm[i]==False:
                max_pool4 = nn.MaxPool1d(self.cnn_max_pool_len[i], stride=None, padding=0, dilation=1)
