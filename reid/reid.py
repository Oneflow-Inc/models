...
from loss import TripletLoss, CrossEntropyLoss
...
parser.add_argument("--gamma", type=float, default=0.1, required=False, help='learning rate decay multiplier')
...
parser.add_argument("--data_dir", type=str, default='./', required=False, help="dataset directory")
...
parser.add_argument("--evaluate", action='store_true', default=False, help="train or eval")
...
parser.add_argument("--load_weights", type=str, default='./resnet50_pretrained_model', help="model load directory")
parser.add_argument("--log_dir", type=str, default="./output", required=False, help="log info save directory")
parser.add_argument("--flow_weight", type=str, default="./output/flow_weight", required=False, help="log info save directory")
...
def main():
    ...
    if not args.evaluate:
        ...
        lambda1 = lambda epoch:    args.lr ** bisect_right(args.step_size, epoch)
...
def train(model, dataset, num_classes, optimizer, scheduler):
    ...
    criterion_x = CrossEntropyLoss(num_classes=num_classes, epsilon=args.epsilon).to("cuda")
    ...
    for epoch in range(0, args.max_epoch):
        ...
        for i in range(len(indicies) // batch_size):
            try:
                ...
                imgs, pids, _ = train_dataset.__getbatch__(indicies[i * batch_size:(i + 1) * batch_size])
            except:
        ...
        print('epoch:',epoch+1,'loss_t:',loss_t.numpy()[0],'loss_x:',loss_x.numpy()[0],'loss:', loss.numpy()[0],   'lr:',optimizer.param_groups[0]['lr'])
...
def evaluate(model, dataset):
    ...
    for i in range((len(query_dataset) // eval_batch) ):
        imgs, pids, camids = query_dataset.__getbatch__(q_ind[i * eval_batch:(i + 1) * eval_batch])
    ...
    for i in range((len(gallery_dataset) // eval_batch)):
        imgs, pids, camids = gallery_dataset.__getbatch__(g_ind[i * eval_batch:(i + 1) * eval_batch])
...
def re_ranking(q_g_dist, q_q_dist, g_g_dist, k1=20, k2=6, lambda_value=0.3):
    ...
    for i in range(all_num):
        ...
        forward_k_neigh_index = initial_rank[i, :k1 + 1]  #找到q_i的k1-neighbors
        backward_k_neigh_index = initial_rank[forward_k_neigh_index, :k1 + 1]  #找到这些k1-neighbors的最近的k1k1-neighbors
        fi = np.where(backward_k_neigh_index == i)[0]  #找到k1k1-neighbors里面含有q_i的
        k_reciprocal_index = forward_k_neigh_index[fi]  #得到k-reciprocal，过滤k1-neighbors
        k_reciprocal_expansion_index = k_reciprocal_index
        for j in range(len(k_reciprocal_index)):
            candidate = k_reciprocal_index[j]
            candidate_forward_k_neigh_index = initial_rank[
                                              candidate, :int(np.around(k1 / 2.)) + 1] #得到k-reciprocal_i的1/2k1-neighbors
            candidate_backward_k_neigh_index = initial_rank[
                                               candidate_forward_k_neigh_index, :int(np.around(k1 / 2.)) + 1] #找到这些1/2k1-neighbors的最近的1/2k1k1-neighbors
            fi_candidate = np.where(
                candidate_backward_k_neigh_index == candidate
            )[0]
            candidate_k_reciprocal_index = candidate_forward_k_neigh_index[
                fi_candidate]#过滤1/2k1-neighbors
            if len(
        ...
        weight = np.exp(-original_dist[i, k_reciprocal_expansion_index])  #赋予权重，不是k_reciprocal_expansion_index的就为0
        V[i, k_reciprocal_expansion_index] = 1. * weight / np.sum(weight)
    original_dist = original_dist[:query_num, ]
    if k2 != 1:
        V_qe = np.zeros_like(V, dtype=np.float32)
        for i in range(all_num):
            V_qe[i, :] = np.mean(V[initial_rank[i, :k2], :], axis=0) #V[i]的k2-neighbors取平均赋给V[i]
        V = V_qe
        del V_qe
    del initial_rank
    invIndex = []
    for i in range(gallery_num):
        invIndex.append(np.where(V[:, i] != 0)[0]) #i作为k-reciprocal
  ...
  #计算jaccard_dist
    for i in range(query_num):
        ...
        indNonZero = np.where(V[i, :] != 0)[0] #q_i的k-reciprocal index
        #indImages = []
