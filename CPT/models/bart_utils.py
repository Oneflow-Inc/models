...
def shift_tokens_right(input_ids: flow.Tensor, pad_token_id: int, decoder_start_token_id: int):
    """
    ...
    shifted_input_ids[:, 0] = flow.tensor(
        decoder_start_token_id, dtype=shifted_input_ids.dtype, device=shifted_input_ids.device)
    ...
    shifted_input_ids = shifted_input_ids.to(flow.float).masked_fill(
        shifted_input_ids.eq(-100).to(flow.int32), pad_token_id).to(flow.int32)

    return shifted_input_ids


def _make_causal_mask(input_ids_shape: flow.Size, dtype: flow.dtype, device: flow.device, past_key_values_length: int = 0):
    """
    ...
    mask = mask.masked_fill(mask_cond < (
