...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, vocab_size, max_position_embeddings, type_vocab_size, hidden_size,
                 layer_norm_eps=1e-5, dropout=0, pad_token_id=0, position_embedding_type="absolute"):
...
class BertLayer(nn.Module):
    def __init__(self, max_position_embeddings, hidden_size, intermediate_size,
        ...
        self.seq_len_dim = 1
        self.attention = BertAttention(max_position_embeddings, hidden_size, nheads,
                                       layer_norm_eps, attn_dropout, hidden_dropout, position_embedding_type, is_decoder)
        self.is_decoder = is_decoder
        self.add_cross_attention = add_cross_attention
        if self.add_cross_attention:
            assert self.is_decoder, f"{self} should be used as a decoder model if cross attention is added"
            self.crossattention = BertAttention(max_position_embeddings, hidden_size, nheads,
                                                layer_norm_eps, attn_dropout, hidden_dropout, position_embedding_type, is_decoder)
...
class BertEncoder(nn.Module):
    def __init__(self, num_layers, max_position_embeddings, hidden_size,
        ...
        self.layer = nn.ModuleList([BertLayer(max_position_embeddings, hidden_size, intermediate_size,  nheads, activation, chunk_size_feed_forward,
...
class Bert(nn.Module):
    def __init__(self, vocab_size=30522, type_vocab_size=2, max_position_embeddings=512,
                 ...
                 layer_norm_eps=1e-12, attn_dropout=0.1, hidden_dropout=0.1,
                 position_embedding_type="absolute", is_decoder=False,
                 add_pooling_layer=True, add_cross_attention=False):
        super(Bert, self).__init__()

        self.embeddings = BertEmbeddings(vocab_size, max_position_embeddings, type_vocab_size,
                                         hidden_size, layer_norm_eps, hidden_dropout, pad_token_id, position_embedding_type)
        self.encoder = BertEncoder(num_layers, max_position_embeddings, hidden_size, intermediate_size, nheads, activation,
                                   chunk_size_feed_forward, layer_norm_eps, attn_dropout, hidden_dropout, position_embedding_type, is_decoder, add_cross_attention)
