...
class PromptPredictor(Predictor):
    ...
    def __init__(
        self, model_dir, model_cls=None, user_defined_parameters=None, *args, **kwargs
    ):
        ...
        self.num_extra_tokens = (
            sum([len(s) if isinstance(s, list) else 0 for s in self.pattern])
            + self.masked_length
        )
        ...
        self.label_candidates_ids = {
            k: str_to_ids(v, self.tokenizer) for k, v in self.label_map.items()
        }
    ...
    def preprocess(self, in_data):
        ...
        for record in in_data:
            ...
            ), "label length should be equal to the mask length"
    ...
    def postprocess(self, result):
        ...
        for b, preds in enumerate(predictions):
            ...
            for pred in preds:
                new_result.append(
                    {"pred": pred["pred"], "log_probability": pred["log_probability"],}
                )
            new_results.append(
                {
...
class CPTPredictor(Predictor):
    ...
    def __init__(
        ...
        self.label_candidates_ids = {
            str_to_ids(v, self.tokenizer)[0]: k for k, v in self.label_map.items()
        }
    ...
    def postprocess(self, result):
        ...
        with torch.no_grad():
            ...
            labels = label_ids[label_ids > 0].detach().cpu().numpy()
            features = (
                torch.nn.functional.normalize(result["features"][label_ids > 0])
                .detach()
                .cpu()
                .numpy()
            )
