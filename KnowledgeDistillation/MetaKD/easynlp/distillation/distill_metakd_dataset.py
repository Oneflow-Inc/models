...
class MetakdSentiClassificationDataset(BaseDataset):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.label_map = dict(
            {value: idx for idx, value in enumerate(self.label_enumerate_values)}
        )
        self.domain_idx_mapping = {
            domain: idx for idx, domain in enumerate(self.domain_list)
        }
    ...
    def readlines_from_file(self, data_file, skip_first_line=None):
        ...
        if self.genre:
            if self.genre in self.domain_list:
                return [
                    row for row in data_rows if row.strip().split("\t")[4] == self.genre
                ]
        else:
            return data_rows
    ...
    def convert_single_row_to_example(self, row):
        ...
        try:
            ...
            raise RuntimeError(
                "Can't load weight in dataset files, \
