...
def export_train_config(
    saved_path, vocab_dir, label_enumerate_values, model_config, cfg
):
    ...
    model_config_dict["pretrained_model_name_or_path"] = pretrained_model_name_or_path
...
def export_label_mapping(saved_path, label_enumerate_values):
    if isinstance(label_enumerate_values, list):
        ...
        if (
            isinstance(label_enumerate_values, str)
            and label_enumerate_values[-1] == "^"
        ):
            io.copy(label_enumerate_values.strip("^"), saved_path)


def export_pytorch_checkpoint_to_tf(
    model,
    ckpt_dir,
