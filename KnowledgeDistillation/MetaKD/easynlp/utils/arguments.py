...
def parse_args(
    ...
    parser = argparse.ArgumentParser(
        description="EasyNLP Arguments", allow_abbrev=False
    )
...
def parse_args_for_cli(
    extra_args_provider=None, defaults={}, ignore_unknown_args=False
):
    """Parse all arguments."""
    parser = argparse.ArgumentParser(
        description="EasyNLP CLI Arguments", allow_abbrev=False
    )
...
def _add_easynlp_args(parser: argparse.ArgumentParser):
    ...
    group.add_argument(
        "--user_script", default=None, type=str, help="The scripts to run with easynlp"
    )
    ...
    group.add_argument(
        ...
        default=None,
        type=str,
        help="The output table, " "output prediction file",
    )

    group.add_argument("--buckets", type=str, default=None, help="Oss buckets")

    group.add_argument(
        "--odps_config", type=str, default=None, help="Config file path of odps"
    )
    ...
    group.add_argument(
        "--max_grad_norm", "--mn", default=1.0, type=float, help="Max grad norm"
    )
    ...
    group.add_argument(
        ...
        default="AdamW",
        type=str,
        choices=["BertAdam", "Adam", "AdamW", "SGD",],
    ...
    group.add_argument(
        "--logging_steps", default=100, type=int, help="logging steps while training"
    )
    group.add_argument(
    ...
    group.add_argument(
        "--output_schema", type=str, default="", help="The schema of the output results"
    )
    group.add_argument(
        "--append_cols", type=str, default=None, help="The schema of the output results"
    )

    group.add_argument(
        "--predict_slice_size", default=4096, type=int, help="Predict slice size"
    )
    group.add_argument(
        "--predict_queue_size", default=1024, type=int, help="Predict queue size"
    )
    group.add_argument(
        "--predict_thread_num", default=2, type=int, help="Predict Thread num"
    )
    group.add_argument(
        "--predict_table_read_thread_num",
        default=16,
        type=int,
    ...
    group.add_argument("--ps_hosts", default="", type=str, help="PS hosts (for PAI-TF)")
    group.add_argument(
        "--chief_hosts", default="", type=str, help="Chief hosts (for PAI-TF)"
    )
    group.add_argument(
        "--job_name", default=None, type=str, help="Name of the job (for PAI-TF)"
    )
    group.add_argument(
        "--task_index", default=0, type=int, help="Index of the task (for PAI-TF)"
    )
    group.add_argument(
        "--task_count", default=1, type=int, help="Number of the task (for PAI-TF)"
    )
    group.add_argument("--is_chief", default="", type=str, help="is chief (for PAI-TF)")

    group.add_argument(
        "--worker_count", default=1, type=int, help="Count of workers/servers"
    )
    group.add_argument(
        "--worker_gpu", default=-1, type=int, help="Count of GPUs in each worker"
    )
    group.add_argument(
        "--worker_cpu", default=-1, type=int, help="Count of CPUs in each worker"
    )
    group.add_argument(
        "--master_port", default=23456, type=int, help="Port of master node"
    )
    group.add_argument(
        "--worker_hosts", default=None, type=str, help="Worker hosts (for PAI-TF)"
    )

    group.add_argument(
        "--use_amp", action="store_true", help="Enable amp, default value is False"
    )
    group.add_argument(
        "--use_torchacc",
        action="store_true",
    ...
    group.add_argument(
        "--data_threads", default=10, type=int, help="Count of CPUs to process data"
    )
