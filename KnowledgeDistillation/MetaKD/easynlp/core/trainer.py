...
class Trainer(object):
    ...
    def set_tensorboard(self):
        ...
        self.tensorboard.add_text(
            tag="config/training", text_string=str(self.args), global_step=0
        )
    ...
    def log_train_infos(self):
        ...
        if args.n_gpu > 0:
            n_tr_samples = (
                len(self._train_loader.dataset) * args.n_gpu
                if args.read_odps
                else len(self._train_loader.dataset)
            )
            n_tr_batch_size = (
                args.micro_batch_size * args.n_gpu * args.gradient_accumulation_steps
            )
        else:
            n_tr_samples = (
        ...
        n_tr_batch_no = (
            len(self._train_loader.dataset) / args.micro_batch_size * args.epoch_num
        )
        ...
        trainable_num_params = sum(
            [
                p.nelement()
                for n, p in self.model_module.named_parameters()
                if p.requires_grad
            ]
        )
        logger.info("  num model params        = %s" % format(model_num_params, ","))
        logger.info(
            "  num trainable params    = %s" % format(trainable_num_params, ",")
        )
        logger.info("\n")
        ...
        if self._lr_scheduler:
            ...
            if hasattr(self._optimizer, "clip_grad_norm"):
                ...
                torch.nn.utils.clip_grad_norm_(
                    self.model_module.parameters(), self.max_grad_norm
                )
    ...
    def after_iter(self, _step, _epoch, loss_dict):
        ...
        if (_step + 1) % args.gradient_accumulation_steps == 0:
            ...
            if (
                self._global_step == 0
                or (self._global_step + 1) % args.logging_steps == 0
            ):
                self._epoch_stats.output(
                    self._global_step + 1, _epoch, self.learning_rate
                )
            ...
            if (
                ...
                and (self._global_step + 1) % args.save_checkpoint_steps == 0
            ):
                ...
                if self.evaluator is not None:
                    ...
                    logger.info(
                        "Best score: {}".format(self.evaluator.best_valid_score)
                    )
                    logger.info(
    ...
    def save_checkpoint(self, save_best=False):
        ...
        if self.args.pretrained_model_name_or_path is not None:
            ...
            if os.path.exists(spiece_path):
                io.copy(
                    spiece_path,
                    os.path.join(
                        get_dir_name(self.args.checkpoint_dir), "spiece.model"
                    ),
                )
        ...
        model_to_save_prefix = (
            "oneflow_model"
            if save_best
            else "oneflow_model_step_%d" % (self._global_step + 1)
        )
        torch.save(
            self.model_module.state_dict(),
            os.path.join(self.args.checkpoint_dir, model_to_save_prefix),
        )
        ...
        torch.save(
            meta_data,
            os.path.join(self.args.checkpoint_dir, model_to_save_prefix) + "_meta",
        )
