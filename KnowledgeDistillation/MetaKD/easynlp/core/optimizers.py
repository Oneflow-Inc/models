...
class EasyNLPWarmupLinearSchedule(LambdaLR):
    ...
    def __init__(self, optimizer, warmup_steps, t_total, last_epoch=-1):
        ...
        # TODO: last——epoch 改为 last_step
        super(EasyNLPWarmupLinearSchedule, self).__init__(
            optimizer, self.lr_lambda, last_step=last_epoch
        )
...
class AdamW(torch.optim.AdamW):
    ...
    def __init__(
        ...
        super(AdamW, self).__init__(params, lr, betas, eps, weight_decay, correct_bias)
    ...
    def get_current_lr(
        self, scheduler: Union[EasyNLPWarmupLinearSchedule, None] = None
    ):
        if scheduler:
...
def get_optimizer(
    ...
    num_train_optimization_steps = int(
        math.ceil(num_steps_per_epoch / gradient_accumulation_steps * epoch_num)
    )
    ...
    if optimizer_type == "BertAdam":
        ...
        if is_torchx_available():
            ...
            optimizer = syncfree.Adam(optimizer_grouped_parameters, lr=learning_rate)
        else:
