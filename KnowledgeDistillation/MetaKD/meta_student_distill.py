...
from easynlp.distillation.distill_metakd_application import (
...
if __name__ == "__main__":
    ...
    teacher = MetaStudentForSequenceClassification(
        pretrained_model_name_or_path=user_defined_parameters["teacher_config_path"],
        num_labels=2,
        num_domains=4,
    )
    # Path to save the model
    teacher.load_state_dict(
        torch.load(user_defined_parameters["teacher_model_path"] + "/oneflow_model")
    )
    print("log: start to load student model...\n")
    if user_defined_parameters["distill_stage"] == "first":
        student = MetaStudentForSequenceClassification(
            pretrained_model_name_or_path=user_defined_parameters[
                "student_config_path"
            ],
            num_labels=2,
            num_domains=4,
        ...
        student.load_state_dict(
            torch.load(user_defined_parameters["student_model_path"] + "/oneflow_model")
        )

    evaluator = SequenceClassificationEvaluator(
        valid_dataset=valid_dataset, user_defined_parameters=user_defined_parameters
    )
    ...
    trainer = MetaDistillationTrainer(
