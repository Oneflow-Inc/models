...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model, dropout=0.1, max_len=5000):
        ...
        div_term = flow.exp(flow.arange(0, d_model, 2).to(
            flow.float) * (-math.log(10000.0) / d_model)).unsqueeze(0)
...
class TransformerModel(nn.Module):
    ...
    def __init__(self, input_sz, output_sz, d_model, nhead, num_encoder_layers, num_decoder_layers, dim_feedforward,
                 dropout):
    ...
    def forward(self, src, tgt, src_mask=None, tgt_mask=None, memory_mask=None,
        ...
        if tgt_mask is None:
            tgt_mask = self.generate_subsequent_mask(
                tgt.shape[0], tgt.shape[0])
