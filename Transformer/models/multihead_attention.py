...
from oneflow.nn import Linear, Module, Parameter
from oneflow import Tensor
from oneflow.nn.init import xavier_uniform_, xavier_normal_, constant_

from .transformer_utils import _in_projection_packed, _scaled_dot_product_attention, linear, _in_projection, pad

from typing import Optional, Tuple

# Multihead attention in torch.nn.activation
...
class MultiheadAttention(Module):
    ...
    bias_k: Optional[flow.Tensor]
    bias_v: Optional[flow.Tensor]
    ...
    def __init__(self, embed_dim, num_heads, dropout=0., bias=True, add_bias_kv=False, add_zero_attn=False,
        ...
        if self._qkv_same_embed_dim is False:
            ...
            self.k_proj_weight = Parameter(flow.zerps((embed_dim, self.kdim)))
    ...
    Shapes for inputs:
        ...
        - key_padding_mask: :math:`(N, S)` where N is the batch size, S is the source sequence length.
          ...
          Must be flow.int8/int32/int64
        - attn_mask: if a 2D mask: :math:`(L, S)` where L is the target sequence length, S is the
          source sequence length. Must be flow.float.
...
def multi_head_attention_forward(
    ...
    if isinstance(embed_dim, flow.Tensor):
    ...
    #
    # contiguous is not allowed in oneflow
    ...
    # expand key_padding_mask and attn_mask if necessary
    if key_padding_mask is not None:
        assert key_padding_mask.shape == (bsz, src_len), \
            f"expecting key_padding_mask shape of {(bsz, src_len)}, but got {key_padding_mask.shape}"
        key_padding_mask = key_padding_mask.reshape((bsz, 1, 1, src_len)).   \
            expand(-1, num_heads, tgt_len, -
                   1).reshape((bsz * num_heads, tgt_len, src_len))
        if attn_mask is not None:
        ...
        if attn_mask is None:
            ...
            # using attn_mask + key_padding_masl to temporarily replace logical_or()
            attn_mask += key_padding_mask
        else:
