...
class VideoDataset(Dataset):
    ...
    def _sample_indices(self, record):
        ...
        if average_duration > 0:
            ...
            offsets = np.sort(
                np.random.randint(
                    record.num_frames - self.old_length + 1, size=self.num_segments
                )
            )
        else:
    ...
    def _get_frames(
        self, record, video_reader, image_tmpl, modality, indices, skip_offsets
    ):
        if self.use_decord:
            ...
            for seg_ind in indices:
                ...
                for i, ind in enumerate(range(0, self.old_length, self.new_step)):
                    ...
                    if self.new_step > 1 and p + self.new_step <= record.num_frames:
    ...
    def __getitem__(self, idx):
        ...
        if self.use_decord:
            video_reader = decord.VideoReader(
                "{}.{}".format(osp.join(self.img_prefix, record.path), self.video_ext)
            )
        ...
        img_group = self._get_frames(
            record, video_reader, image_tmpl, modality, segment_indices, skip_offsets
        )
