...
def parse_args():
    ...
    parser.add_argument('--data_dir',
                        ...
                        help='data file path')
    parser.add_argument('--pretrained', default='/home/liling/work/oneflow/TSN/weights/resnet50_imagenet_pretrain_model',
    ...
    parser.add_argument('--lr_steps', default=[40, 80],
                        help='lr step')
    parser.add_argument('--batch_size', default=4,
                        help='imgs per gpu')
    parser.add_argument('--save_checkpoint_path', default="/home/liling/work/oneflow/TSN/res",
                        help='imgs per gpu')
    parser.add_argument('--lr', default=0.0025,
                        help='learning_rate')
    parser.add_argument('--mom', default=0.9,
                        help='momentum')
    parser.add_argument('--weight_decay', default=0.000,
                        help='weight decay')
    ...
    parser.add_argument('--spatial_feature_size', default=1,
                        help='data normalization value')
    parser.add_argument('--dropout_ratio', default=0.4,
                        help='data normalization value')
...
def main():
    ...
    train_ann_file = args.data_dir + "/kinetics400/kinetics400_val_list_videos_sub_frames.txt"
    train_prefix = args.data_dir + "/kinetics400/rawframes_val"
    val_ann_file = args.data_dir + "/kinetics400/kinetics400_val_list_videos_sub_frames.txt"
    val_prefix = args.data_dir + "/kinetics400/rawframes_val"

    train_dataset = TSNDataSet("", train_ann_file, num_segments=3,
                   video_dir=train_prefix,
                   new_length=1,
                   image_tmpl='img_{:05d}.jpg',
                   flip = GroupMultiScaleCrop(224, [1, .875, .75, .66]),
    ...
    val_dataset = TSNDataSet("", val_ann_file, num_segments=3,
    ...
    for epoch in range(args.epochs):
        ...
        for idx in range(int(len(train_dataset) / args.batch_size)):
            ...
            num_modalities = 1
            img_group = data.reshape(args.batch_size, int(data.shape[0] / args.batch_size), data.shape[1],
            ...
            if idx % print_interval == 0:
                ...
                print('epoch {} train iter {} oneflow loss {}, lr: {}, train time : {}'.format(epoch, idx, l, tmp_lr, end_t - start_t))
        ...
        if epoch % val_interval == 0:
            ...
            print("epoch %d, oneflow top1 val acc: %f, top5 val acc: %f" % (epoch, top1*100, top5*100))
            flow.save(model.state_dict(), os.path.join(args.save_checkpoint_path, "epoch_%d_val_acc_%f_%f" % (epoch, top1, top5)))
