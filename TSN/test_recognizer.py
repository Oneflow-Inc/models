...
def parse_args():
    ...
    parser.add_argument("--dropout_ratio", default=0.0, help="data normalization value")
...
def multi_test(model, data_loader):
    ...
    for idx in range(int(len(data_loader) / args.batch_size)):
        ...
        # print(int(data.shape[0]/args.batch_size))
    ...
    #     # print(data)
    #     # print(data['img_group_0'].data[0].shape)
    #     # img_group = data.reshape(data.shape[0], int(data.shape[1]/3), 3, data.shape[2], data.shape[3]).numpy()
    #     # img_group = data.reshape(args.batch_size, int(data.shape[0] / args.batch_size / 3), 3, data.shape[1], data.shape[2]).numpy()
    #     img_group = flow.Tensor(data.numpy(), device=flow.device("cuda"))
    #     # print(img_group)
    #     with flow.no_grad():
    #         result = model(num_modalities, img_meta, img_group)
    #     results.append(result)
...
def main():
    ...
    cropping = torchvision.transforms.Compose([GroupOverSample(224, 256)])
