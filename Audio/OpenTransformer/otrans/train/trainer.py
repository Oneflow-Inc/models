...
from otrans.train.utils import MeanLoss, AverageMeter, Summary, map_to_cuda, AuxiliaryLossAverageMeter
import pickle
import shutil
...
class Trainer(object):
    def __init__(self, params, model, optimizer, scheduler, is_visual=False,
                 expdir='./', ngpu=1, local_rank=0, is_debug=False,
            ...
            if self.global_training_step % self.accum_steps == 0:
                ...
                if self.grad_noise > 0.0:
                    for p in self.model.parameters():
                        if p.requires_grad:
                            noise =flow.tensor(np.random.normal(0, self.grad_noise, p.grad.shape,),device=loss.device)
    ...
    def load_model(self, checkpoint):
        ...
        if 'model.pt' in chkpt:
            ...
            self.model.encoder.load_state_dict(flow.load(os.path.join(checkpoint, 'encoder.pt')))
            self.model.decoder.load_state_dict(flow.load(os.path.join(checkpoint, 'decoder.pt')))
        if 'frontend.pt' in chkpt:
            self.model.frontend.load_state_dict(
                flow.load(os.path.join(checkpoint, 'frontend.pt')))
        if 'ctc.pt' in chkpt:
            self.model.assistor.load_state_dict(flow.load(os.path.join(checkpoint, 'ctc.pt')))
