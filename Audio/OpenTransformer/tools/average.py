...
def average_chkpt(datadir, start, end):
        ...
        for k, v in states.items():
            ...
            try:
                ...
                if 'batch_norm.num_batches_tracked' in k:
                    averaged_params[key][k] = flow.div(averaged_params[key][k], num_models).long()
                else:
    ...
    flow.save(new_state['frontend'], os.path.join(datadir, 'model.average.from%sto%s.pt' % (start, end), 'frontend.pt'))
    flow.save(new_state['encoder'], os.path.join(datadir, 'model.average.from%sto%s.pt' % (start, end),'encoder.pt'))
    flow.save(new_state['decoder'], os.path.join(datadir, 'model.average.from%sto%s.pt' % (start, end),'decoder.pt'))
    print('Save the average checkpoint as %s' % os.path.join(datadir, 'model.average.from%sto%s.pt' % (start, end)))
    print('Done!')


shutil.copytree(os.path.join(rootdir, 'model.epoch.%s.pt' % st, 'params.tar'),
