...
def get_args():
    parser = argparse.ArgumentParser("""Infer CycleGAN using sample dataset""")
    parser.add_argument('--logf0s_normalization', type=str,
                        help="Cached location for log f0s normalized", default='./cache/logf0s_normalization.npz')
    parser.add_argument('--mcep_normalization', type=str,
                        help="Cached location for mcep normalization", default='./cache/mcep_normalization.npz')
    parser.add_argument('--coded_sps_A_norm', type=str,
                        help="mcep norm for data A", default='./cache/coded_sps_A_norm.pickle')
    parser.add_argument('--coded_sps_B_norm', type=str,
                        help="mcep norm for data B", default='./cache/coded_sps_B_norm.pickle')
    ...
    parser.add_argument('--validation_A_dir', type=str,
                        help="validation set for sound source A", default='./data/S0913/')
    parser.add_argument('--output_A_dir', type=str,
                        help="output for converted Sound Source A", default='./converted_sound/S0913')
    parser.add_argument('--validation_B_dir', type=str,
                        help="Validation set for sound source B", default='./data/gaoxiaosong/')
    parser.add_argument('--output_B_dir', type=str,
                        help="Output for converted sound Source B", default='./converted_sound/gaoxiaosong/')
    parser.add_argument('--pretrain_models', type=str,
                        help="Path of pretrain models", default='./pretrain_models')
    parser.add_argument('--infer_data', type=str,
                        help="Path of infer data", default='./sample')
...
def infer(argv):
    ...
    cycleGAN = CycleGANTrainr(logf0s_normalization=argv.logf0s_normalization,
    ...
    cycleGAN.loadModel(PATH=argv.pretrain_models)
    cycleGAN.infer(PATH=argv.infer_data)
