...
def copy_files(source_dir, target_dir, file_dir_name_list):
    for file_dir_name in file_dir_name_list:
        if os.path.exists(os.path.join(target_dir, file_dir_name)):
            continue
        shutil.copytree(os.path.join(source_dir, file_dir_name),
...
def wav_to_mcep_file(dataset: str, sr=SAMPLE_RATE, processed_filepath: str = './data/processed'):
    ...
    for one_speaker in d.keys():
        ...
        for index, one_chunk in enumerate(chunks(values_of_one_speaker, CHUNK_SIZE)):
            ...
            f0, ap, sp, coded_sp = cal_mcep(
                wav_concated, sr=sr, dim=FEATURE_DIM)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_files', type=list, help='original datasets',
                        default=['vcc2016_training.zip', 'evaluation_all.zip'])
    parser.add_argument('--train_dir', type=str,
                        help='trainset directory', default='./data/speakers')
    parser.add_argument('--test_dir', type=str,
                        help='testset directory', default='./data/speakers_test')
    parser.add_argument('--output_dir', type=str,
                        help='the directory stores the processed data', default='./data/processed')
    parser.add_argument("--speaker_ids", type=list, default=[
                        "SF1", "SF2", "TM1", "TM2"], help="Source speaker id from VCC2016.")
