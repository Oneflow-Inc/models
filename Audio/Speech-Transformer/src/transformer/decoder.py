    ...
    def recognize_beam(self, encoder_outputs, char_list, args):
        ...
        for i in range(maxlen):
            ...
            for hyp in hyps:
                ...
                # -- Prepare masks
                non_pad_mask = flow.ones_like(ys).to(dtype=flow.float32).unsqueeze(-1)
                ...
                for j in range(beam):
                    ...
                    new_hyp['yseq'] = flow.ones(1, (1 + ys.size(1))).type_as(
                        encoder_outputs).long()
                    new_hyp['yseq'][:, :ys.size(1)] = hyp['yseq']
                    new_hyp['yseq'][:, ys.size(1)] = int(float(
                        local_best_ids[0, j].numpy()))
            ...
            if i == maxlen - 1:
                for hyp in hyps:
                    hyp['yseq'] = flow.cat([hyp['yseq'],flow.ones(1, 1).fill_(self.eos_id).type_as(encoder_outputs).long()],dim=1)
            ...
            if len(hyps) > 0:
                ...
                break
            for hyp in hyps:
                print('hypo: ' + ''.join([char_list[int(x.numpy())]
