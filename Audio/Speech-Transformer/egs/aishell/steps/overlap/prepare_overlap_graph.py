...
def get_args():
    ...
    args.min_states_overlap = int(args.min_overlap_duration / args.frame_shift + 0.5)
    args.max_states_overlap = int(args.max_overlap_duration / args.frame_shift + 0.5)
...
def print_states(args, file_handle):
    ...
    print(
        "0 {state} single single {cost}".format(
    ...
    print(
        "{sil_state} {speech_state} single single {cost}".format(
    ...
    for state in range(
        ...
        print(
            "{state} {next_state} single single {cost}".format(
                state=state, next_state=state + 1, cost=0.0
            ),
            file=file_handle,
        )
    ...
    for state in range(
        ...
        print(
            "{state} {ovl_state} overlap overlap {cost}".format(
                state=state,
                ovl_state=overlap_start_state,
                cost=-math.log(args.transition_probability),
            ),
            file=file_handle,
        )
    ...
    print(
        "{state} {ovl_state} overlap overlap {cost}".format(
            state=speech_last_state, ovl_state=overlap_start_state, cost=0
        ),
        file=file_handle,
    )
    ...
    for state in range(
        overlap_start_state, overlap_start_state + args.min_states_overlap - 1
    ):
        print(
            "{state} {next_state} overlap overlap {cost}".format(
                state=state, next_state=state + 1, cost=0.0
            ),
            file=file_handle,
        )
    ...
    for state in range(
        ...
        print(
            "{state} {speech_state} single single {cost}".format(
                state=state,
                speech_state=speech_start_state,
                cost=-math.log(args.transition_probability),
            ),
    ...
    print(
        "{state} {speech_state} single single {cost}".format(
            state=overlap_last_state, speech_state=speech_start_state, cost=0.0
        ),
        file=file_handle,
    )
    ...
    for state in range(overlap_start_state, overlap_last_state + 1):
        print("{state} {cost}".format(state=state, cost=0), file=file_handle)
