...
def run(args):
    ...
    with common_lib.smart_open(args.overlap_rttm) as f:
        for line in f.readlines():
            parts = line.strip().split()
            segments.append(
                Segment(parts[1], float(parts[3]), dur=float(parts[4]), label=parts[7])
            )
    ...
    for reco_id in reco2num_frames:
        ...
        for seg in segs:
            ...
            end_frame = min(
                int(seg.end_time / args.frame_shift), reco2num_frames[reco_id]
            )
    ...
    with common_lib.smart_open(args.out_targets_ark, "w") as f:
        for reco_id in sorted(reco2targets.keys()):
            common_lib.write_matrix_ascii(
                f, reco2targets[reco_id].tolist(), key=reco_id
            )
