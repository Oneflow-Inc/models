...
class CallBackModelCheckpoint(object):
    ...
    def __call__(self, global_step, backbone, fc7 ):
        if global_step > 100 and self.rank == 0:
            path_module = os.path.join(self.output, "backbone.pth")
            flow.save(backbone.module.state_dict(), path_module)
            logging.info("Pytorch Model Saved in '{}'".format(path_module))

        if global_step > 100 and fc7 is not None:
            path_module = os.path.join(self.output, "fc7.pth")
            flow.save(fc7.module.state_dict(), path_module)
            logging.info("Pytorch Model Saved in '{}'".format(path_module))
