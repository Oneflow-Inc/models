...
def save_ds_checkpoint(iteration, model, lr_scheduler, args, tag):
    ...
    torch.save(model.state_dict(), os.path.join(args.save, str(iteration)+"_glm_model.pt"))
...
def _build_key_size_numel_dictionaries(keys, data):
    ...
    sizes_cpu = torch.Tensor(sizes).to(torch.int64)
...
def get_loss(vocab_parallel_logits,target):
    ...
    masked_target_1d = masked_target.view((-1,)).to(torch.int)

    arange_1d = torch._C.arange(start=0, end=logits_2d.size()[0],
                                device=logits_2d.device).to(torch.int)
