...
class GLMForMultiTokenCloze(flow.nn.Module):
    ...
    def forward(self, input_ids, position_ids, attention_mask, target_ids=None, logit_mask=None, prompt_pos=None):
        ...
        batch_ids = flow._C.arange(target_ids.size(0), dtype=flow.long, device=target_ids.device)
        batch_ids = batch_ids.unsqueeze(1).expand_as(target_ids)
        seq_ids = flow._C.arange(target_ids.size(-1), dtype=flow.long, device=target_ids.device)
...
class GLMForMultiTokenClozeFast(flow.nn.Module):
    ...
    def forward(self, input_ids, position_ids, attention_mask,
        ...
        def build_dec_mask_matrix(seq_length, sep, memory_length=0):
            ...
            ids = flow._C.arange(memory_length, device=sep.device, dtype=sep.dtype).view(1, -1)
        ...
        batch_ids = flow._C.arange(dec_target_ids.size(0), dtype=flow.long, device=dec_target_ids.device)
        batch_ids = batch_ids.unsqueeze(1).expand_as(dec_target_ids)
        seq_ids = flow._C.arange(dec_target_ids.size(-1), dtype=flow.long, device=dec_target_ids.device)
...
class GLMForSingleTokenCloze(flow.nn.Module):
    ...
    def forward(self, input_ids, position_ids, attention_mask, target_ids=None, logit_mask=None, prompt_pos=None):
        ...
        outputs, *mems = self.model(input_ids, position_ids, attention_mask, prompt_pos=prompt_pos)
        batch_ids = flow._C.arange(outputs.size(0), dtype=attention_mask.dtype, device=attention_mask.device)
...
class GLMForSequenceClassification(flow.nn.Module):
    ...
    def forward(self, input_ids, position_ids, attention_mask):
        ...
        if self.pool_token == 'start':
            output = outputs[
                flow._C.arange(outputs.size(0), dtype=attention_mask.dtype, device=attention_mask.device), attention_mask]
        elif self.pool_token == 'pad':
            output = outputs[flow._C.arange(outputs.size(0), dtype=attention_mask.dtype,
