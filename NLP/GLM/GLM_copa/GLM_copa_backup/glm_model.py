...
class VocabParallelEmbedding(torch.nn.Module):
    ...
    def forward(self, input_):
        ...
        torch._oneflow_internal.profiler.RangePush('VocabParallelEmbedding-1')
        ...
        torch._oneflow_internal.profiler.RangePush('VocabParallelEmbedding-2')
        masked_input = (input_.clone() - self.vocab_start_index).to(torch.int)
        torch._oneflow_internal.profiler.RangePop()
        
        #不支持切片索引
        torch._oneflow_internal.profiler.RangePush('VocabParallelEmbedding-3')
        masked_input[input_mask] = 0
        torch._oneflow_internal.profiler.RangePop()
        torch._oneflow_internal.profiler.RangePush('VocabParallelEmbedding-4')
        output_parallel = F.embedding(masked_input, 
                                      ...
                                      self.sparse)
        torch._oneflow_internal.profiler.RangePop()
        torch._oneflow_internal.profiler.RangePush('VocabParallelEmbedding-5')                              
        #不支持切片索引
...
class GPT2ParallelTransformer(torch.nn.Module):
    ...
    def forward(self, hidden_states, position_ids, attention_mask, memory_states=None, encoder_states=None,
        ...
        torch.ones((16,16,16,16))
        torch.zeros((16,16,16,16))
...
class GLMModel(torch.nn.Module):
    ...
    def forward(self, input_ids, position_ids, attention_mask, *mems, return_memory=False, detach_memory=True,
        ...
        torch._oneflow_internal.profiler.RangePush('embeddings')
        ...
        # Transformer.
        torch._oneflow_internal.profiler.RangePush('transformer')
...
class GLMForMultiTokenCloze(torch.nn.Module):
    ...
    def forward(self, input_ids, position_ids, attention_mask, target_ids=None, logit_mask=None, prompt_pos=None):
        ...
        if self.take_softmax:
            torch._oneflow_internal.profiler.RangePush('logsoftmax')
...
def test():
    ...
    def np_data(shape):
        tensor = torch.ones(shape).to(torch.int64).cuda()
        return tensor
    ...
    with torch.no_grad():
        ...
        b = time.time()
        for i in range(100):
            torch._oneflow_internal.profiler.RangePush('item')
