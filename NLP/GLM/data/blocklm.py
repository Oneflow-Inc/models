...
class ConstructBlockStrategy:
    def __init__(self, args, tokenizer, max_seq_length, bert_prob=1.0, gap_sentence_prob=0.0, gpt_infill_prob=0.5,
        ...
        self.block_length_distribution = [poisson.pmf(
            i, average_block_length) for i in range(1, max_block_length)]
    ...
    def make_block_data(self, tokens, loss_masks, attention_mask, block_spans, rng, task='bert'):
        ...
        for start, end, idx in block_spans:
            ...
            if not self.sentinel_token:
                ...
                target_position_ids.append(
                    [self.max_seq_length] * (end - start + 1))
        ...
        if self.encoder_decoder:
            ...
            if task == 'bert' and self.context_mask_ratio > 0:
                ...
                mask_pos = rng.sample(mask_candidates, int(
                    self.context_mask_ratio * text_length))
                for pos in mask_pos:
        ...
        for sample in samples:
            ...
            for _ in range(num_splits):
                if target_length >= len(tokens):
                    ...
                    random_start = rng.randrange(
                        0, len(tokens) - target_length)
                    ...
                    if random_end - random_start < target_length // 2:
                        random_end = random_start + target_length
                    new_tokens, new_loss_masks = tokens[random_start:
                                                        random_end], loss_masks[random_start: random_end]
                new_tokens = np.concatenate(([cls_id], new_tokens))
                new_loss_masks = np.concatenate(([0], new_loss_masks))
                new_samples.append(
                    {'text': new_tokens, 'loss_mask': new_loss_masks})
        return new_samples
    ...
    def construct_blocks(self, samples):
        ...
        worker_id, num_workers = 0, 1
        rng = random.Random(
            (self.count * num_workers + worker_id) * self.world_size + self.rank)
        self.count += 1
        ...
        if rand < self.bert_prob:
            ...
            for sample in samples:
                ...
                if self.masked_lm:
                    ...
                    sep = len(sample['text']) - \
                        masked_count + len(masked_lengths)
            ...
            for sample in samples:
                ...
                for start, end in sentence_spans:
                    ...
                    if block_length >= int(self.gap_sentence_ratio * len(tokens)):
                        break
                data = self.make_block_data(
                    tokens, loss_masks, None, block_spans, rng, task='gap_sentence')
            ...
            for sample in samples:
                generation_length = min(
                    max_generation_length, len(sample['text']) - 2)
                ...
                if multiple_doc or rng.random() < self.infill_prob:
                    ...
                    tokens = np.concatenate((
                        source_tokens, [self.generation_mask,
                                        self.tokenizer.get_command('sop').Id],
                        target_tokens[:-1]))
                    targets = np.concatenate(
                        (source_tokens, [self.generation_mask], target_tokens))
                    loss_masks = np.concatenate(
                        (np.zeros(len(source_tokens) + 1, dtype=np.long), target_masks))
                    token_batch.append(tokens)
                    target_batch.append(targets)
                    loss_mask_batch.append(loss_masks)
                    position_ids = np.arange(
                        len(source_tokens) + len(target_tokens) + 1, dtype=np.long)
