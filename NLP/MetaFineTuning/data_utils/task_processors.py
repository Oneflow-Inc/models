...
class TextClassificationProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        """See base class."""
        return self._create_examples(pd.read_csv(os.path.join(data_dir, "train.csv"), sep='\t', header=None).values.tolist(),
                                     "train")

    def get_dev_examples(self, data_dir):
        """See base class."""
        return self._create_examples(pd.read_csv(os.path.join(data_dir, "dev.csv"), sep='\t', header=None).values.tolist(), "dev")

    def get_test_examples(self, data_dir):
        """See base class."""
        return self._create_examples(pd.read_csv(os.path.join(data_dir, "test.csv"), sep='\t', header=None).values.tolist(), "test")
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            # elif self.task_name in ['mr', 'sst-5', 'subj', 'trec', 'cr', 'mpqa', 'g1']:
...
class Sst2Processor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        """See base class."""
        return self._create_examples(pd.read_csv(os.path.join(data_dir, "train.csv"), sep='\t', header=None).values.tolist(), "train")
...
class G2Processor(DataProcessor):
    ...
    def get_test_examples(self, data_dir):
        """See base class."""
        return self._create_examples(pd.read_csv(os.path.join(data_dir, "test.csv"), sep='\t', header=None).values.tolist(),
                                     "test")
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            try:
                ...
                continue
            examples.append(InputExample(guid=guid, text_a=text_a, text_b=line[2], label=label))
        return examples


PROCESSORS = {
    "g1": TextClassificationProcessor, # cross task group 1
