...
from classifier import MFTBERT, BERT
import tokenization
...
parser.add_argument("--task_name", type=str, default='sst-2', choices=['sst-2', 'mr', 'cr'])
...
num_eval_steps = math.ceil(args.dev_example_num / eval_batch_size)
...
def BertGlueEvalValJob():
    _, logits, label_ids = BuildBert(
        ...
        os.path.join(args.data_dir, 'ofrecord', 'eval'),
...
def main():
        ...
        for step in range(epoch_size):
            ...
            if global_step % args.dev_every_step_num == 0:
                ...
                dev_acc = run_eval_job(
                    dev_job_func=BertGlueEvalDevJob,
                ...
                if best_dev_acc < dev_acc:
                    ...
                    print('===== saving model ... =====')
                    print('saving model ...')
                    snapshot.save("best_ft_model_{}_dev_{}".format(args.task_name, best_dev_acc))
    ...
    snapshot.load(os.path.join(snapshot._model_save_dir, "snapshot_best_ft_model_{}_dev_{}".format(args.task_name, best_dev_acc)))
