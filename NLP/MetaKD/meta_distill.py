...
from classifier import MetaStudentBERT
import tokenization
from util import Snapshot, InitNodes, Metric, CreateOptimizer, GetFunctionConfig
from data_utils.task_processors3 import processors, convert_examples_to_features, generate_dataset
...
parser.add_argument("--teacher_model", type=str, default='', required=True, help="The path of meta-teacher.")
parser.add_argument("--student_model", type=str, default='', required=True, help="The path of student model.")
...
def BertDecoder(
    ...
    with flow.scope.placement("cpu", "0:0"):
        ...
        _blob_conf("logit_blob", [seq_length], dtype=flow.float32)
        _blob_conf("all_attention_scores_blob", [seq_length * hidden_size], dtype=flow.float32)
        _blob_conf("pooled_output", [seq_length * hidden_size], dtype=flow.float32)
        return blob_confs
...
def BuildBert(
    ...
    loss, logits = MetaStudentBERT(
        ...
        input_logit_blob=decoders['logit_blob'],
        input_attention_blob=decoders['all_attention_scores_blob'],
        input_pooled_blob=decoders['pooled_output'],
...
def main():
    ...
    InitNodes(args)
    snapshot = Snapshot(args.model_save_dir, args.teacher_model)
