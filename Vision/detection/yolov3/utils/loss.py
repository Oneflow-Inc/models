...
class ComputeLoss:
    ...
    def __call__(self, p, targets):  # predictions, targets, model
        ...
        lcls, lbox, lobj = flow.zeros(1, device=device), flow.zeros(1, device=device), flow.zeros(1, device=device)
        tcls, tbox, indices, anchors = self.build_targets(p, targets)  # targets

        # Losses
        for i, pi in enumerate(p):  # layer index, layer predictions
            b, a, gj, gi = indices[i]  # image, anchor, gridy, gridx
            tobj = flow.zeros_like(pi[..., 0], device=device)  # target obj
            ...
            if n:
                ...
                if self.sort_obj_iou:
                    sort_id = flow.argsort(score_iou)
                    b, a, gj, gi, score_iou = b[sort_id], a[sort_id], gj[sort_id], gi[sort_id], score_iou[sort_id]
                tobj[b, a, gj, gi] = (1.0 - self.gr) + self.gr * score_iou  # iou ratio

                # Classification
                if self.nc > 1:  # cls loss (only if multiple classes)
                    t = flow.full_like(ps[:, 5:], self.cn, device=device)  # targets
    ...
    def build_targets(self, p, targets):
        ...
        ai = flows.arange(na, device=targets.device).float().view(na, 1).repeat(1, nt)  # same as .repeat_interleave(nt)
        ...
        for i in range(self.nl):
            ...
            gain[2:6] = flow.tensor(p[i].shape)[[3, 2, 3, 2]]  # xyxy gain
