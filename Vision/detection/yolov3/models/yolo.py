...
from utils.flow_utils import model_info, scale_img, initialize_weights
...
class Detect(nn.Module):
    ...
    def __init__(self, nc=80, anchors=(), ch=(), inplace=True):  # detection layer
        ...
        self.anchor_grid = flow.Tensor(a.clone().view(self.nl, 1, -1, 1, 1, 2))
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            ...
            if not self.training:  # inference
                ...
                self.anchor_grid = self.anchor_grid.to(y.device)
...
class Model(nn.Module):
    def __init__(self, cfg='yolov3.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        if isinstance(m, Detect):
            ...
            # logger.info('Stride: %s' % m.stride.tolist())
    ...
    def _initialize_biases(self, cf=None):  # initialize biases into Detect(), cf is class frequency
        ...
        for mi, s in zip(m.m, m.stride):  # from
            ...
            b.data[:, 4] += 8 / (640 / s) ** 2  # obj (8 objects per 640 image)
            b.data[:, 5:] += math.log(0.6 / (m.nc - 0.99)) if cf is None else flow.log(cf / cf.sum())  # cls
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, numebr, module, args
        ...
        if m in [Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, DWConv, CrossConv, BottleneckCSP, C3]:
            c1, c2 = ch[f], args[0]
