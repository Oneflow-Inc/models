...
from models.common import attempt_load
...
def save_one_txt(predn, save_conf, shape, file):
    # Save one txt result
    gn = flow.tensor(shape)[[1, 0, 1, 0]]  # normalization gain whwh
...
def run(data,
    ...
    if training:  # called by train.py
        ...
        # Load model
        model = attempt_load(weights, cfg="models/yolov3.yaml", device=device)
        stride = model.stride
    ...
    iouv = flow.linspace(0.5, 0.95, 10).to(device)  # iou vector for mAP@0.5:0.95
    niou = iouv.numel()

    # Dataloader
    if not training:
        if pt and device.type != 'cpu':
            model(flow.zeros(1, 3, imgsz, imgsz).to(device).type_as(next(model.model.parameters())))  # warmup
    ...
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        for si, pred in enumerate(out):
            ...
            if nl:
                ...
                correct = flow.zeros(pred.shape[0], niou, dtype=flow.bool)
