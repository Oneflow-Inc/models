...
class ConfusionMatrix:
    ...
    def process_batch(self, detections, labels):
        ...
        for i, gc in enumerate(gt_classes):
            ...
            if n and sum(j) == 1:
                self.matrix[detection_classes[m1[j].tolist()], gc] += 1  # correct
            else:
...
def box_iou(box1, box2, eps=1e-7):
    ...
    (a1, a2), (b1, b2) = box1[:, None].contiguous().chunk(2, 2), box2.contiguous().chunk(2, 1)
    inter = (flow.min(a2, b2) - flow.max(a1, b1)).clamp(0).prod(2)

    # inter(N,M) = (rb(N,M,2) - lt(N,M,2)).clamp(0).prod(2)

    return inter / (box_area(box1.T)[:, None] + box_area(box2.T) - inter + eps)  # iou = inter / (area1 + area2 - inter)
