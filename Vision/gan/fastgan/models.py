...
class GLU(nn.Module):
    def forward(self, x):
        ...
        nc = int(nc/2)
        return x[:, :nc] * flow.sigmoid(x[:, nc:])

class Swish(nn.Module):
    def forward(self, feat):
        return feat * flow.sigmoid(feat)

class InitLayer(nn.Module):
    def __init__(self, nz, channel):
        super().__init__()
        self.init = nn.Sequential(convTranspose2d(nz, channel*2, 4, 1, 0, bias=False), batchNorm2d(channel*2), GLU())
