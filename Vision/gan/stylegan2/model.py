...
class EqualConv2d(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.weight = nn.Parameter(flow.randn(out_channel, in_channel, kernel_size, kernel_size))
        self.scale = 1 / math.sqrt(in_channel * kernel_size ** 2)
        self.stride = stride
        self.padding = padding

        if bias:
            self.bias = nn.Parameter(flow.zeros(out_channel))
        else:
            self.bias = None
...
class ConvLayer(nn.Sequential):
    def __init__(
        ...
        layers.append(
            EqualConv2d(in_channel, out_channel, kernel_size, padding=self.padding, stride=stride, bias=bias and not activate,)
        )

        if activate:
            layers.append(nn.LeakyReLU(out_channel, bias=bias))
