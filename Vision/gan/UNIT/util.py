...
def get_data_loader_list(root, file_list, batch_size, train, new_size=None,
    ...
    loader = flow.utils.data.DataLoader(dataset=dataset, batch_size=batch_size, shuffle=train, drop_last=True, num_workers=num_workers)
    return loader
...
def get_scheduler(optimizer, hyperparameters, iterations=-1):
    if 'lr_policy' not in hyperparameters or hyperparameters['lr_policy'] == 'constant':
        ...
        scheduler = lr_scheduler.StepLR(optimizer, step_size=hyperparameters['step_size'],
                                        gamma=hyperparameters['gamma'], last_step=iterations)
    else:
...
def __write_images(image_outputs, display_image_num, file_name):
    ...
    image_tensor = flow.cat([images[:display_image_num] for images in image_outputs], 0)
