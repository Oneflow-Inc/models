...
class PixelNorm(nn.Module):
    ...
    def forward(self, input):
        return input * flow.rsqrt(flow.mean(input ** 2, dim=2, keepdim=True) + 1e-8)
...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., window_size=16):
        ...
        self.relative_position_bias_table = nn.Parameter(
            flow.zeros((2 * window_size - 1) * (2 * window_size - 1), num_heads))  # 2*Wh-1 * 2*Ww-1, nH

        # get pair-wise relative position index for each token inside the window
        coords_h = flow.arange(window_size)
        coords_w = flow.arange(window_size)
        ...
        self.noise_strength_1 = flow.nn.Parameter(flow.zeros([]))

        nn.init.trunc_normal_(self.relative_position_bias_table, std=.02)

    def forward(self, x):
        B, N, C = x.shape
        x = x + flow.randn([x.size(0), x.size(1), 1], device=x.device) * self.noise_strength_1
...
class Generator(nn.Module):
    def __init__(self, args, img_size=224, patch_size=16, in_chans=3, num_classes=10, embed_dim=384, depth=5,
        ...
        self.pos_embed_1 = nn.Parameter(flow.zeros(1, self.bottom_width ** 2, embed_dim))
        self.pos_embed_2 = nn.Parameter(flow.zeros(1, (self.bottom_width * 2) ** 2, embed_dim))
        self.pos_embed_3 = nn.Parameter(flow.zeros(1, (self.bottom_width * 4) ** 2, embed_dim))
        self.pos_embed_4 = nn.Parameter(flow.zeros(1, (self.bottom_width * 8) ** 2, embed_dim // 4))
        self.pos_embed_5 = nn.Parameter(flow.zeros(1, (self.bottom_width * 16) ** 2, embed_dim // 16))
        self.pos_embed_6 = nn.Parameter(flow.zeros(1, (self.bottom_width * 32) ** 2, embed_dim // 64))

        self.embed_pos = nn.Parameter(flow.zeros(1, self.bottom_width ** 2, embed_dim))
        ...
        self.blocks_2 = StageBlock(depth=depth[1], dim=embed_dim, embedding_dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale, drop=drop_rate,
            attn_drop=attn_drop_rate, drop_path=0, act_layer=act_layer, norm_layer=norm_layer, window_size=16)
        self.blocks_3 = StageBlock(depth=depth[2], dim=embed_dim, embedding_dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
            drop=drop_rate, attn_drop=attn_drop_rate, drop_path=0, act_layer=act_layer, norm_layer=norm_layer, window_size=32)
        self.blocks_4 = StageBlock(depth=depth[3], dim=embed_dim // 4, embedding_dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale, drop=drop_rate,
            attn_drop=attn_drop_rate, drop_path=0, act_layer=act_layer, norm_layer=norm_layer, window_size=self.window_size)
        self.blocks_5 = StageBlock(depth=depth[4], dim=embed_dim // 16, embedding_dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale, drop=drop_rate,
            attn_drop=attn_drop_rate, drop_path=0, act_layer=act_layer, norm_layer=norm_layer, window_size=self.window_size)
        self.blocks_6 = StageBlock(depth=depth[5], dim=embed_dim // 64, embedding_dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale, drop=drop_rate,
            attn_drop=attn_drop_rate, drop_path=0, act_layer=act_layer, norm_layer=norm_layer, window_size=self.window_size)
...
class Discriminator(nn.Module):
    def __init__(self, args, img_size=32, patch_size=None, in_chans=3, num_classes=1, embed_dim=None, depth=7,
        ...
        self.pos_embed_1 = nn.Parameter(flow.zeros(1, num_patches_1, embed_dim // 8))
        self.pos_embed_2 = nn.Parameter(flow.zeros(1, num_patches_2, embed_dim // 4))
        self.pos_embed_3 = nn.Parameter(flow.zeros(1, num_patches_3, embed_dim // 2))
        self.pos_embed_4 = nn.Parameter(flow.zeros(1, num_patches_4, embed_dim))
