...
class Generator(nn.Module):
    def __init__(self, args, img_size=224, patch_size=16, in_chans=3, num_classes=10, embed_dim=384, depth=5,
        ...
        # dpr = [x.item() for x in flow.linspace(0, drop_path_rate, depth[0])]  # stochastic depth decay rule
        dpr = [
            x for x in np.linspace(0, drop_path_rate, sum(depth))
        ]
    ...
    def forward(self, z, epoch):
        ...
        for index, blk in enumerate(self.upsample_blocks):
            ...
            x = x + self.pos_embed[index+1].cuda()
            x = blk(x)
            # _, _, H, W = x.size()
            # x = x.view(-1, self.embed_dim, H*W)
            # x = x.permute(0,2,1)
