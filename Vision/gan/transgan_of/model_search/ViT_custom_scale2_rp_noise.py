...
from model_search.vit_helper import DropPath, trunc_normal_
# from model_search.diff_aug import DiffAugment
from flowvision.utils import make_grid, save_image

# from model_search.ada import *
import scipy.signal
...
class Generator(nn.Module):
    def __init__(self, args, img_size=224, patch_size=16, in_chans=3, num_classes=10, embed_dim=384, depth=5,
        ...
        self.pos_embed_2 = nn.Parameter(flow.zeros(1, (self.bottom_width*2)**2, embed_dim//4))
        self.pos_embed_3 = nn.Parameter(flow.zeros(1, (self.bottom_width*4)**2, embed_dim//16))
        self.pos_embed = [
            self.pos_embed_1,
            self.pos_embed_2,
            self.pos_embed_3
        ...
        dpr = [
            x for x in np.linspace(0, drop_path_rate, depth[0])
        ]
...
#         num_patches_4 = ((args.img_size//8) // patch_size)**2
        ...
        self.pos_embed_1 = nn.Parameter(flow.zeros(1, num_patches_1, embed_dim//4*3))
        self.pos_embed_2 = nn.Parameter(flow.zeros(1, num_patches_2, embed_dim))
...
#         trunc_normal_(self.pos_embed_4, std=.02)
        ...
        if 'filter' in self.args.diff_aug:
            ...
            Hz_fbank = flow.as_tensor(Hz_fbank, dtype=flow.float32)
            self.register_buffer('Hz_fbank', flow.as_tensor(Hz_fbank, dtype=flow.float32))
        else:
            self.Hz_fbank = None
    ...
    def forward_features(self, x, aug=False, epoch=400): # aug需要改为True
        if "None" not in self.args.diff_aug and aug:
            pass
            # x = DiffAugment(x, self.args.diff_aug, True, [self.Hz_geom, self.Hz_fbank]) # 需要使用
#         with torch.no_grad():
