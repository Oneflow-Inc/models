...
import numpy as np
from .network_helpers.linear import FC, Conv2d
from .network_helpers.norm import PixelNorm, InstanceNorm
from .network_helpers.noise import ApplyNoise
from .network_helpers.style import ApplyStyle
from .network_helpers.upscale import Upscale2d
...
class Blur2d(nn.Module):
    def __init__(self, f=[1,2,1], normalize=True, flip=False, stride=1):
        ...
        if f is not None:
            f = flow.tensor(f, dtype=flow.float32)
            f = f[:, None] * f[None, :]
            f = f[None, None]
            if normalize:
                f = f / f.sum()
...
class G_synthesis(nn.Module):
    def __init__(self,
        ...
        for layer_idx in range(num_layers):
            ...
            self.noise_inputs.append(flow.randn(*shape).to("cuda"))
        ...
        # Initial Input Block
        self.const_input = nn.Parameter(flow.ones(1, self.nf(1), 4, 4))
        self.bias = nn.Parameter(flow.ones(self.nf(1)))
...
class StyleGenerator(nn.Module):
    ...
    def forward(self, latents1):
        ...
        if self.truncation_psi and self.truncation_cutoff:
            ...
            dlatents1 = dlatents1 * flow.Tensor(coefs).to(dlatents1.device)
